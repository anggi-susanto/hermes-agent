...
class _SdkModelFallback:
    """Tiny kwargs container used when optional Alibaba SDK models are absent.

    Unit tests inject mocked SDK clients without installing the full DingTalk
    card/robot model packages.  The mocked clients only need request objects
    with attributes, not real SDK classes, so this keeps the AI-card path
    testable while production still uses the real models when available.
    """
...
def _sdk_model(module: Any, name: str) -> Any:
    if module is not None:
        model = getattr(module, name, None)
        if model is not None:
            return model
    return _SdkModelFallback


def _runtime_options() -> Any:
    if tea_util_models is not None:
...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            CreateCardRequest = _sdk_model(dingtalk_card_models, "CreateCardRequest")
            CreateCardRequestCardData = _sdk_model(
                dingtalk_card_models, "CreateCardRequestCardData"
            )
            CreateCardRequestImGroupOpenSpaceModel = _sdk_model(
                dingtalk_card_models, "CreateCardRequestImGroupOpenSpaceModel"
            )
            CreateCardRequestImRobotOpenSpaceModel = _sdk_model(
                dingtalk_card_models, "CreateCardRequestImRobotOpenSpaceModel"
            )
            CreateCardHeaders = _sdk_model(dingtalk_card_models, "CreateCardHeaders")

            # Step 1: Create card with STREAM callback type
            create_request = CreateCardRequest(
                card_template_id=self._card_template_id,
                out_track_id=out_track_id,
                card_data=CreateCardRequestCardData(
            ...
            create_headers = CreateCardHeaders(
            ...
            DeliverCardRequest = _sdk_model(dingtalk_card_models, "DeliverCardRequest")
            DeliverCardRequestImGroupOpenDeliverModel = _sdk_model(
                dingtalk_card_models, "DeliverCardRequestImGroupOpenDeliverModel"
            )
            DeliverCardRequestImRobotOpenDeliverModel = _sdk_model(
                dingtalk_card_models, "DeliverCardRequestImRobotOpenDeliverModel"
            )
            DeliverCardHeaders = _sdk_model(dingtalk_card_models, "DeliverCardHeaders")
            if is_group:
                open_space_id = f"dtv1.card//IM_GROUP.{conversation_id}"
                deliver_request = DeliverCardRequest(
                    out_track_id=out_track_id,
                    user_id_type=1,
            ...
            deliver_headers = DeliverCardHeaders(
                x_acs_dingtalk_access_token=token,
            )
    ...
    async def _stream_card_content(
        ...
        StreamingUpdateRequest = _sdk_model(dingtalk_card_models, "StreamingUpdateRequest")
        StreamingUpdateHeaders = _sdk_model(dingtalk_card_models, "StreamingUpdateHeaders")
        stream_request = StreamingUpdateRequest(
            out_track_id=out_track_id,
            guid=str(uuid.uuid4()),
            key="content",
        ...
        stream_headers = StreamingUpdateHeaders(
            x_acs_dingtalk_access_token=token,
        )
...
class _IncomingHandler(
    ...
    async def process(self, message: "CallbackMessage"):
        ...
        try:
            ...
            # Parse dict into ChatbotMessage using SDK's from_dict. Tests and
            # optional-dependency-light installs may exercise this handler
            # without dingtalk-stream installed; keep the conversion path
            # functional with a small attribute container instead of failing on
            # None.from_dict.
            if ChatbotMessage is not None:
                ...
                chatbot_msg = SimpleNamespace(
                    ...
                    sender_id=data.get("senderId") if isinstance(data, dict) else None,
                    sender_staff_id=data.get("senderStaffId") if isinstance(data, dict) else None,
                    conversation_id=data.get("conversationId") if isinstance(data, dict) else None,
                    conversation_type=data.get("conversationType") if isinstance(data, dict) else None,
                    message_id=data.get("msgId") if isinstance(data, dict) else None,
                    session_webhook=(
                        data.get("sessionWebhook")
                        or data.get("session_webhook")
                        or ""
                    ) if isinstance(data, dict) else "",
                    is_in_at_list=bool(data.get("isInAtList")) if isinstance(data, dict) else False,
                    raw=data,
                )
