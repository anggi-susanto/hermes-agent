...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _is_dm_channel(self, channel: Any) -> bool:
        """Best-effort check for Discord DM channels.

        Full-suite tests intentionally replace the shared discord module mock in
        several files. Falling back to the class name keeps channel-type checks
        stable when mock class identity changes between collection and runtime.
        """
        if channel is None:
            return False
        ...
        if dm_cls and isinstance(channel, dm_cls):
            return True
        return channel.__class__.__name__.endswith("DMChannel")

    def _is_thread_channel(self, channel: Any) -> bool:
        """Best-effort check for Discord thread channels.

        Mirrors _is_dm_channel's class-name fallback to avoid test/mock order
        dependence while preserving normal discord.py isinstance behavior.
        """
        if channel is None:
            return False
        ...
        if thread_cls and isinstance(channel, thread_cls):
            return True
        return "Thread" in channel.__class__.__name__
