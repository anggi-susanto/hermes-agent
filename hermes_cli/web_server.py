...
#
# Each subscriber gets an asyncio.Queue; /api/pub enqueues events and
# /api/events owns all WebSocket sends. Sending from the publisher task directly
# to a subscriber WebSocket races with Starlette TestClient under xdist because
# the subscriber handler is simultaneously blocked in receive_text().
_event_channels: dict[str, set[asyncio.Queue[str]]] = {}
_event_backlog: dict[str, list[str]] = {}
_EVENT_BACKLOG_LIMIT = 128
...
def _enqueue_event(queue: asyncio.Queue[str], payload: str) -> None:
    try:
        ...
        # A stuck subscriber should not block the PTY publisher. Drop the
        # oldest frame and keep the latest tool event moving.
        try:
            queue.get_nowait()
...
async def _broadcast_event(channel: str, payload: str) -> None:
    ...
    async with _event_lock:
        backlog = _event_backlog.setdefault(channel, [])
        backlog.append(payload)
        del backlog[:-_EVENT_BACKLOG_LIMIT]
...
async def events_ws(ws: WebSocket) -> None:
    ...
    async with _event_lock:
        ...
        for payload in _event_backlog.get(channel, ()):
            _enqueue_event(queue, payload)
