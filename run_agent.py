...
class AIAgent:
    ...
    def __init__(
        ...
        if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_invoke", "bedrock_converse", "codex_app_server"}:
        ...
        self._cache_ttl = "5m"
        try:
            _pc_cfg = (self.config or {}).get("prompt_caching", {}) or {}
        ...
        # Keep this unconditional: even quiet/skip_memory unit-test agents use
        # config-derived knobs (context_length, tool-use policy, retries, aux
        # compression overrides). Heavy startup probes can be skipped separately
        # without dropping semantic config.
        try:
            from hermes_cli.config import load_config as _load_agent_config
        ...
        skip_startup_compression_check = bool(
            skip_memory
            and self.quiet_mode
            and not self._aux_compression_context_length_config
        )
        if not skip_startup_compression_check:
            # Quiet/background agents with memory explicitly disabled are often
            # created in high-volume cache churn tests.  They can skip the aux
            # probe unless config explicitly provided an auxiliary compression
            # context override that must be validated at startup.
                ...
                if review_agent is not None:
                    try:
                        with open(os.devnull, "w", encoding="utf-8") as _fn, \
                            ...
                            try:
                                _session_messages = getattr(review_agent, "_session_messages", []) or []
                                try:
                                    review_agent.shutdown_memory_provider(_session_messages)
                                except TypeError:
                                    review_agent.shutdown_memory_provider()
                            except Exception:
                                pass
