...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    "SMS_ALLOW_ALL_USERS",
    # API server toggles are not credential-shaped except for API_SERVER_KEY,
    # but they alter load_gateway_config() results. Clear them so config tests
    # that assert explicit YAML false values are not polluted by the developer
    # shell or earlier tests on the same xdist worker.
    "API_SERVER_ENABLED",
    "API_SERVER_HOST",
    "API_SERVER_PORT",
