...
def _ensure_discord_mock():
    """Install or refresh mock discord modules for e2e adapter tests.

    Several Discord test modules replace ``sys.modules["discord"]`` with their
    own partial mocks during the full suite.  These e2e helpers need a stable
    DMChannel/Thread identity that also matches ``gateway.platforms.discord``;
    otherwise DMs can be misclassified as server channels and dropped by the
    mention gate.
    """
    if "discord" in sys.modules and hasattr(sys.modules["discord"], "__file__"):
        return sys.modules["discord"]  # Real library installed
