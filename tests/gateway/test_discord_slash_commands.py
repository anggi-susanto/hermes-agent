...
def _sync_discord_module_global():
    """Keep the adapter's cached discord module aligned with this test mock.

    Other full-suite gateway tests can import gateway.platforms.discord after
    installing a partial mock. Re-sync both sys.modules and the module-global
    before each slash-command test so registration sees app_commands.Command and
    app_commands.autocomplete consistently.
    """
    _ensure_discord_mock()
    import gateway.platforms.discord as discord_adapter

    discord_adapter.discord = sys.modules["discord"]
    discord_adapter.DISCORD_AVAILABLE = True
    thread_cls = globals().get("_FakeThreadChannel")
    if thread_cls is not None:
        discord_adapter.discord.Thread = thread_cls
