...
def _make_adapter():
    ...
    # The adapter acquires a machine-local scoped lock keyed by session path.
    # Keep each bypassed-__init__ adapter isolated so xdist workers/tests don't
    # collide on a shared /tmp/test-wa-session identity.
    adapter._session_path = Path(tempfile.mkdtemp(prefix=f"test-wa-session-{os.getpid()}-"))
