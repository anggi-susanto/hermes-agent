...
class TestPtyWebSocket:
    ...
    def test_pub_broadcasts_to_events_subscribers(self, monkeypatch):
        ...
        # Use a unique channel so leftovers from another xdist worker cannot
        # satisfy the registry wait while this subscriber is still between
        # accept() and queue registration.
        channel = f"broadcast-test-{time.monotonic_ns()}"
        ...
        with self.client.websocket_connect(sub_path) as sub:
            # Wait for *this* channel's subscriber queue to be registered on
            # the server side. websocket_connect returns when ws.accept()
            # completes, but the server adds us to ``_event_channels`` in a
            # follow-up await, so publishing immediately can race ahead and
            # drop the message.
            deadline = time.monotonic() + 5.0
            while time.monotonic() < deadline:
