...
def get_check_fn_cache_generation() -> int:
    """Return a monotonic token that changes when availability checks reset."""
    with _check_fn_cache_lock:
        return _check_fn_cache_generation


def is_check_fn_cache_empty() -> bool:
    """Return True when no availability checks are currently memoized."""
    with _check_fn_cache_lock:
        return not _check_fn_cache
...
def invalidate_check_fn_cache() -> None:
    """Drop cached ``check_fn`` results and bump the availability generation.

    Call after config or environment changes that affect tool availability
    (e.g. ``hermes tools enable`` or tests monkeypatching terminal backend
    requirements). The generation token lets higher-level schema caches key on
    availability invalidation even before any new check_fn result is stored.
    """
...
class ToolRegistry:
    ...
    def get_availability_fingerprint(self) -> tuple:
        """Return a cheap fingerprint for check_fn bindings on registered tools.

        Tests and plugin reloads may monkeypatch or re-register a tool's
        availability function without changing the registry generation. Schema
        caches key on this token so those binding changes cannot reuse stale
        tool definitions from a previous availability implementation.
        """
        return tuple(
            sorted(
                (entry.name, id(entry.check_fn) if entry.check_fn is not None else None)
    ...
    def register(
        ...
        with self._lock:
            ...
            if existing:
                ...
                existing_mod = getattr(existing.handler, "__module__", None)
                new_mod = getattr(handler, "__module__", None)
                same_module_refresh = existing_mod == new_mod
                if both_mcp:
                    logger.debug(
                        "Tool '%s': MCP toolset '%s' overwriting MCP toolset '%s'",
                        name, toolset, existing.toolset,
                    )
                elif existing.toolset != toolset or not same_module_refresh:
                    # Reject shadowing — prevent plugins/MCP/direct file imports
                    # from overwriting built-ins. The same tool module may
                    # still refresh its own registration (e.g. importlib.reload).
                    logger.error(
                        "Tool registration REJECTED: '%s' (toolset '%s', module '%s') would "
                        "shadow existing tool from toolset '%s' (module '%s'). Deregister the "
                        "existing tool first if this is intentional.",
                        name, toolset, new_mod, existing.toolset, existing_mod,
                    )
                    return
